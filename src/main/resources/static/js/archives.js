...
var localhostPath = curPath.substring(0, pos);
    ...
    function putInArchivesArticleInfo(data){
        ...
        $.each(data['result'], function (index, obj) {
            ...
            content.append($('<header class="am-comment-hd" >' +
                '<div class="contentTitle am-comment-meta">' +
                '<a href="'+localhostPath+'/article/details/' + obj['articleId'] + '">' + obj['articleTitle'] + '</a>' +
                '</div>' +
                '</header>'));
            var amCommentBd = $('<div class="am-comment-bd"></div>');
            amCommentBd.append($('<i class="am-icon-calendar"> <a >' + obj['publishDate'] + '</a></i>' +
                '<i class="am-icon-folder"> <a href="'+localhostPath+'/category/1?category=' + obj['articleCategories'] + '">' + obj['articleCategories'] + '</a></i>'));
            var amCommentBdTags = $('<i class="am-comment-bd-tags am-icon-tag"></i>');
            for(var i=0;i<obj['articleTags'].length;i++){
                var tag = $('<a href="'+localhostPath+'/article/index/1?tag=' + obj['articleTags'][i] + '">' + obj['articleTags'][i] + '</a>');
